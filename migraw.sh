...
function spawn_zsh {
    ...
    if [ "$1" != "" ]; then
        env -i PHPRC=$PHPRC PHP_INI_SCAN_DIR=$PHP_INI_SCAN_DIR TERM=$TERM SSH_AUTH_SOCK=$SSH_AUTH_SOCK MYSQL_HOME=$MYSQL_HOME PATH=$PATH COMPOSER_HOME=$COMPOSER_HOME NODE_HOME=$NODE_HOME NPM_CONFIG_PREFIX=$NPM_CONFIG_PREFIX HOME=$HOME zsh -c $1
    else
...
EOL
        ...
        env -i ZDOTDIR=$MIGRAW_CURRENT/shell PHPRC=$PHPRC PHP_INI_SCAN_DIR=$PHP_INI_SCAN_DIR TERM=$TERM SSH_AUTH_SOCK=$SSH_AUTH_SOCK MYSQL_HOME=$MYSQL_HOME PATH=$PATH COMPOSER_HOME=$COMPOSER_HOME NODE_HOME=$NODE_HOME NPM_CONFIG_PREFIX=$NPM_CONFIG_PREFIX HOME=$HOME zsh
    fi
}
