...
function install {
    ...
    do
        ...
        if [ "$PHP_VERSION" == "7.2" ]; then
            # until this is fixed use 7.2.18, that was the latest working version
            # https://bugs.php.net/bug.php?id=78106
            # https://bugs.php.net/bug.php?id=78101
            wget -q -O $DOWNLOAD/php-7.2.zip https://windows.php.net/downloads/releases/archives/php-7.2.18-Win32-VC15-x64.zip
            continue
        fi
...
EOL
    ...
    if [ "$1" != "" ]; then
        env -i WSLENV=$WSLENV PHPRC=$PHPRC PHP_INI_SCAN_DIR=$PHP_INI_SCAN_DIR TERM=$TERM SSH_AUTH_SOCK=$SSH_AUTH_SOCK MYSQL_HOME=$MYSQL_HOME PATH=$PATH COMPOSER_HOME=$COMPOSER_HOME SystemDrive=$SystemDrive HOME=$HOME bash -c "$1"
    else
        env -i WSLENV=$WSLENV PHPRC=$PHPRC PHP_INI_SCAN_DIR=$PHP_INI_SCAN_DIR TERM=$TERM SSH_AUTH_SOCK=$SSH_AUTH_SOCK MYSQL_HOME=$MYSQL_HOME PATH=$PATH COMPOSER_HOME=$COMPOSER_HOME SystemDrive=$SystemDrive HOME=$HOME bash --rcfile <(echo ' PS1="'$(echo $PROMPT)' "')
    fi;
}
