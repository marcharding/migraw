...
function spawn_bash {
    ...
    if [ "$1" != "" ]; then
        env -i MYSQL_UNIX_PORT=$MYSQL_UNIX_PORT NPM_CONFIG_USERCONFIG=$NPM_CONFIG_USERCONFIG NPM_CONFIG_CACHE=$NPM_CONFIG_CACHE GEM_SPEC_CACHE=$GEM_SPEC_CACHE NODE_PATH=$NODE_PATH NPM_CONFIG_PREFIX=$NPM_CONFIG_PREFIX PHP_INI_SCAN_DIR=$PHP_INI_SCAN_DIR TERM=$TERM SSH_AUTH_SOCK=$SSH_AUTH_SOCK RUBYLIB=$RUBYLIB GEM_PATH=$GEM_PATH GEM_HOME=$GEM_HOME PATH="$PATH" LD_LIBRARY_PATH=$LD_LIBRARY_PATH PHPRC=$PHPRC HOME=$HOME bash --rcfile <(echo 'PS1="'"$MIGRAW_USER@$MIGRAW_YAML_name"':\w\$ "') -c "$1"
    else
        env -i MYSQL_UNIX_PORT=$MYSQL_UNIX_PORT NPM_CONFIG_USERCONFIG=$NPM_CONFIG_USERCONFIG NPM_CONFIG_CACHE=$NPM_CONFIG_CACHE GEM_SPEC_CACHE=$GEM_SPEC_CACHE NODE_PATH=$NODE_PATH NPM_CONFIG_PREFIX=$NPM_CONFIG_PREFIX PHP_INI_SCAN_DIR=$PHP_INI_SCAN_DIR TERM=$TERM SSH_AUTH_SOCK=$SSH_AUTH_SOCK RUBYLIB=$RUBYLIB GEM_PATH=$GEM_PATH GEM_HOME=$GEM_HOME PATH="$PATH" LD_LIBRARY_PATH=$LD_LIBRARY_PATH PHPRC=$PHPRC HOME=$HOME bash --rcfile <(echo 'PS1="'$(echo $PROMPT)' "')
    fi;
}
...
function spawn_zsh {
    ...
    if [ "$1" != "" ]; then
        env -i MYSQL_UNIX_PORT=$MYSQL_UNIX_PORT ZDOTDIR=$MIGRAW_CURRENT/shell PHPRC=$PHPRC PHP_INI_SCAN_DIR=$PHP_INI_SCAN_DIR TERM=$TERM SSH_AUTH_SOCK=$SSH_AUTH_SOCK MYSQL_HOME=$MYSQL_HOME PATH="$PATH" COMPOSER_HOME=$COMPOSER_HOME HOME=$HOME LD_LIBRARY_PATH=$LD_LIBRARY_PATH zsh -c "$1"
    else
...
EOL
        ...
        env -i MYSQL_UNIX_PORT=$MYSQL_UNIX_PORT ZDOTDIR=$MIGRAW_CURRENT/shell PHPRC=$PHPRC PHP_INI_SCAN_DIR=$PHP_INI_SCAN_DIR TERM=$TERM SSH_AUTH_SOCK=$SSH_AUTH_SOCK MYSQL_HOME=$MYSQL_HOME PATH="$PATH" COMPOSER_HOME=$COMPOSER_HOME HOME=$HOME LD_LIBRARY_PATH=$LD_LIBRARY_PATH zsh
    fi
}
